...
export function calculateMetrics(
  trades: Array<{ pnl: DecimalLike | number | null; rMultiple?: DecimalLike | number | null }>
): PerformanceMetrics {
  ...
  // Sharpe Ratio (simplified — uses daily P&L std dev)
  let sharpeRatio: number | null = null;
  if (totalTrades > 1) {
    const mean = avgPnl;
    const variance = pnls.reduce((acc, p) => acc + Math.pow(p - mean, 2), 0) / (totalTrades - 1);
